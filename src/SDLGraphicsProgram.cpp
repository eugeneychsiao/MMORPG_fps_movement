...
void SDLGraphicsProgram::Loop(){
    ...
    // std::cout << heightMap.GetPixelR(500,125) << std::endl;
    // std::cout << myTerrain->getHeight(125.0f,500.0f) << std::endl;
    ...
    while(!quit){
        ...
        while(SDL_PollEvent( &e ) != 0){
            ...
            if(e.type==SDL_MOUSEMOTION){
                ...
                if (mouseX < 1 || mouseX > 1250 || mouseY < 1 || mouseY > 710){
                    SDL_WarpMouseInWindow(m_window, 640,360);
                } else {
                    if (mouseX != 640 && mouseY != 360){
                    m_renderer->GetCamera(0)->SetCameraViewPosition(m_renderer->GetCamera(0)->GetViewXDirection(), m_renderer->GetCamera(0)->GetViewYDirection(), m_renderer->GetCamera(0)->GetViewZDirection());
                    m_renderer->GetCamera(0)->MouseLook(m_window, mouseX, mouseY);
                    }
                }
                // std::cout << m_renderer->GetCamera(0)->GetViewXDirection() << std::endl;
                m_renderer->GetCamera(0)->MouseLook(m_window, mouseX, mouseY);
            }
            switch(e.type){
                ...
                case SDL_KEYDOWN:
                    switch(e.key.keysym.sym){
                        case SDLK_LEFT:
                            ...
                            // m_renderer->GetCamera(0)->SetCameraEyePosition(m_renderer->GetCamera(0)->GetEyeXPosition(),myTerrain->getHeight(m_renderer->GetCamera(0)->GetEyeXPosition(),m_renderer->GetCamera(0)->GetEyeZPosition())+25,m_renderer->GetCamera(0)->GetEyeZPosition());
                            // m_renderer->GetCamera(0)->GetEyeXPosition();
                            // m_renderer->GetCamera(0)->GetEyeZPosition();
                            break;
                        case SDLK_RIGHT:
